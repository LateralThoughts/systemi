angular.module('activity', ['ui.bootstrap', 'ngResource', 'ngRoute', 'client-select', 'default-values', 'day-block', 'invoice-modal', 'activity-modal'])
    ...
    .controller('ListCtrl', ['$scope', '$http', '$location', '$modal', 'ActivityModalService', function ($scope, $http, $location, $modal, activityModalService) {
        ...
        function reloadActivities() {
            $http.get("/api/activities").success(function (data) {
                _.map(data, function (activity) {
                    activity.firstDay = moment(activity.activity.days[0].day, "YYYY-MM-DD").format("L");
                    activity.lastDay = moment(activity.activity.days[activity.activity.days.length - 1].day, "YYYY-MM-DD").format("L");
                });
        ...
        $scope.deleteActivity = function (id) {
            $http.delete("/api/activity/" + id).success(function () {
                reloadActivities()
            });
        };
    ...
    .controller('CreateCtrl', ['$scope', '$modal', '$log', '$http', 'Client', 'default_contractor',
        function ($scope, $modal, $log, $http, Client, default_contractor) {
            ...
            $scope.cra = {days: [], contractor: default_contractor.name};
