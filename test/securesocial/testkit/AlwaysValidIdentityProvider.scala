...
class AlwaysValidIdentityProvider extends IdentityProvider {
  ...
  override def authenticate()(implicit request: Request[play.api.mvc.AnyContent]): Future[AuthenticationResult] = {
    val oauth2Info = new OAuth2Info("aaaa", None, None, None)

    Future(Authenticated(BasicProfile("google", id, Some("jean"), Some("test"), Some("jean test"), Some("jean.test@lateral-thoughts.com"), None, authMethod, None, Some(oauth2Info), None)))
  }
